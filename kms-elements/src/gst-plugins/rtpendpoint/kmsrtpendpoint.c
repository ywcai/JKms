...
#define KMS_RTP_ENDPOINT_GET_PRIVATE(obj)                                      \
  (G_TYPE_INSTANCE_GET_PRIVATE (                                               \
      (obj), KMS_TYPE_RTP_ENDPOINT, KmsRtpEndpointPrivate))
...
enum { PROP_0, PROP_USE_SDES, PROP_MASTER_KEY, PROP_CRYPTO_SUITE };
...
sdes_ext_data_new (KmsRtpEndpoint *ep, const gchar *media)
{
  ...
  kms_ref_struct_init (
      KMS_REF_STRUCT_CAST (edata), (GDestroyNotify) sdes_ext_data_destroy);
...
sdes_keys_new (KmsISdpMediaExtension *ext)
{
  ...
  kms_ref_struct_init (
      KMS_REF_STRUCT_CAST (keys), (GDestroyNotify) sdes_keys_destroy);
...
static gboolean
kms_rtp_endpoint_set_local_srtp_connection_key (
    KmsRtpEndpoint *self, const gchar *media, SdesKeys *sdes_keys)
{
  ...
  kms_srtp_connection_set_key (
      KMS_SRTP_CONNECTION (sdes_keys->conn), key, auth, cipher, TRUE);
  g_free (key);
...
static void
kms_rtp_endpoint_set_remote_srtp_connection_key (
    KmsRtpEndpoint *self, const gchar *media, SdesKeys *sdes_keys)
{
  ...
  kms_srtp_connection_set_key (
      KMS_SRTP_CONNECTION (sdes_keys->conn), rem_key, auth, cipher, FALSE);
    ...
    KmsBaseSdpEndpoint *base_sdp, gint id, KmsSdpSession **sess)
{
  ...
  g_object_get (self, "use-ipv6", &use_ipv6, NULL);
  if (self->priv->use_sdes) {
    *sess = KMS_SDP_SESSION (
        kms_srtp_session_new (base_sdp, id, manager, use_ipv6));
  } else {
    *sess =
  ...
  /* Chain up */
  KMS_BASE_SDP_ENDPOINT_CLASS (kms_rtp_endpoint_parent_class)
...
kms_rtp_endpoint_on_offer_keys_cb (KmsSdpSdesExt *ext, SdesExtData *edata)
{
  ...
  if (!kms_rtp_endpoint_create_new_key (self, DEFAULT_KEY_TAG, &key)) {
    GST_ERROR_OBJECT (
        self, "Can not generate master key for media %s", edata->media);
    KMS_ELEMENT_UNLOCK (self);
...
kms_rtp_endpoint_is_supported_key (KmsRtpEndpoint *self, GValue *key)
{
  ...
  if (!kms_sdp_sdes_ext_get_parameters_from_key (
          key, KMS_SDES_CRYPTO, G_TYPE_UINT, &crypto, NULL)) {
    return FALSE;
  }
...
static gboolean
kms_rtp_endpoint_on_answer_keys_cb (
    KmsSdpSdesExt *ext, const GArray *keys, GValue *key, SdesExtData *edata)
{
  KmsRtpEndpoint *self = KMS_RTP_ENDPOINT (edata->rtpep);
  SdesKeys *sdes_keys;
  GValue *offer_key;
  ...
  if (!kms_sdp_sdes_ext_get_parameters_from_key (
          offer_key, KMS_SDES_TAG_FIELD, G_TYPE_UINT, &tag, NULL)) {
...
static void
kms_rtp_endpoint_on_selected_key_cb (
    KmsSdpSdesExt *ext, const GValue *key, SdesExtData *edata)
{
  KmsRtpEndpoint *self = KMS_RTP_ENDPOINT (edata->rtpep);
  SdesKeys *sdes_keys;
...
kms_rtp_endpoint_provide_sdes_handler (KmsRtpEndpoint *self, const gchar *media)
{
  ...
  if (!kms_sdp_media_handler_add_media_extension (
          handler, KMS_I_SDP_MEDIA_EXTENSION (ext))) {
    GST_ERROR_OBJECT (
        self, "Can not use SDES in handler %" GST_PTR_FORMAT, handler);
    goto end;
  }
    ...
    const gchar *media, KmsSdpMediaHandler **handler)
{
  if (g_strcmp0 (media, "audio") == 0 || g_strcmp0 (media, "video") == 0) {
    *handler =
        kms_rtp_endpoint_get_media_handler (KMS_RTP_ENDPOINT (base_sdp), media);
  }

  /* Chain up */
  KMS_BASE_SDP_ENDPOINT_CLASS (kms_rtp_endpoint_parent_class)
      ->create_media_handler (base_sdp, media, handler);
}

/* Media handler management end */

static void
kms_rtp_endpoint_configure_connection_keys (
    KmsRtpEndpoint *self, KmsRtpBaseConnection *conn, const gchar *media)
{
  SdesKeys *sdes_keys;
  ...
  if (!kms_rtp_endpoint_set_local_srtp_connection_key (
          self, media, sdes_keys)) {
    ...
    KmsSdpSession *sess, KmsSdpMediaHandler *handler, GstSDPMedia *media)
{
  ...
  /* Chain up */
  ret = KMS_BASE_SDP_ENDPOINT_CLASS (kms_rtp_endpoint_parent_class)
  ...
  conn = kms_rtp_endpoint_get_connection (
      KMS_RTP_ENDPOINT (base_sdp_endpoint), sess, handler, media);
  ...
  if (self->priv->use_sdes) {
    kms_rtp_endpoint_configure_connection_keys (
        self, conn, gst_sdp_media_get_media (media));
  }
...
static void
kms_rtp_endpoint_comedia_on_ssrc_active (
    GObject *rtpsession, GObject *rtpsource, KmsRtpEndpoint *self)
{
  ...
  g_object_get (rtpsource, "ssrc", &ssrc, "is-validated", &is_validated,
      "is-sender", &is_sender, NULL);

  if (!is_validated || !is_sender) {
    GST_DEBUG_OBJECT (
        rtpsession, "Ignore uninteresting RTPSource, SSRC: %u", ssrc);
    return;
  }
  ...
  ok = gst_structure_get (
      source_stats, "rtp-from", G_TYPE_STRING, &rtp_from, NULL);
  if (!ok) {
  ...
  ok = gst_structure_get (
      source_stats, "rtcp-from", G_TYPE_STRING, &rtcp_from, NULL);
  if (!ok) {
  ...
  gulong signal_id = GPOINTER_TO_UINT (
      g_hash_table_lookup (self->priv->comedia.signal_ids, rtpsession));

  GST_INFO_OBJECT (
      rtpsession, "COMEDIA: Disconnect from signal 'on_ssrc_active'");
...
static void
kms_rtp_endpoint_comedia_manager_create (
    KmsRtpEndpoint *self, const GstSDPMedia *media, KmsRtpBaseConnection *conn)
{
  ...
  g_hash_table_insert (
      self->priv->comedia.rtp_conns, g_object_ref (rtpsession), conn);
    ...
    KmsSdpSession *sess, gboolean offerer)
{
  ...
  /* Chain up */
  KMS_BASE_SDP_ENDPOINT_CLASS (parent_class)
      ->start_transport_send (base_sdp_endpoint, sess, offerer);
  ...
  for (guint i = 0; i < len; i++) {
    ...
    if (media_con == NULL || media_con->address == NULL
      ...
      const gchar *media_str = gst_sdp_media_get_media (media);
      GST_WARNING_OBJECT (
          self, "Missing connection information for '%s'", media_str);
      continue;
    }
    ...
    gboolean comedia_enabled =
        g_strcmp0 (
            gst_sdp_media_get_attribute_val (media, "direction"), "active")
        == 0;
    if (comedia_enabled) {
      const gchar *media_str = gst_sdp_media_get_media (media);
      ...
      const gchar *media_str = gst_sdp_media_get_media (media);
      GST_INFO_OBJECT (
          self, "COMEDIA:  Media '%s' doesn't use COMEDIA", media_str);

      guint rtp_port = gst_sdp_media_get_port (media);
      guint rtcp_port = rtp_port + 1;

      const gchar *attr_rtcp_port =
          gst_sdp_media_get_attribute_val (media, "rtcp");
      if (attr_rtcp_port != NULL) {
        rtcp_port = (guint) atoi (attr_rtcp_port);
      }

      kms_rtp_base_connection_set_remote_info (
          conn, media_con->address, (gint) rtp_port, (gint) rtcp_port);
    }
  }
}

static void
kms_rtp_endpoint_set_property (
    GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
  ...
  case PROP_CRYPTO_SUITE:
    ...
    self->priv->use_sdes = self->priv->crypto != KMS_RTP_SDES_CRYPTO_SUITE_NONE;
    break;
  default:
...
kms_rtp_endpoint_class_init (KmsRtpEndpointClass *klass)
{
  ...
  GST_WARNING ("=============IS THE RTP ENDPOINT CLASS IS INIT===============");
  ...
  gst_element_class_set_details_simple (gstelement_class, "RtpEndpoint",
      "RTP/Stream/RtpEndpoint", "Rtp Endpoint element",
  ...
  g_object_class_install_property (gobject_class, PROP_USE_SDES,
      g_param_spec_boolean ("use-sdes", "Use SDES",
          "Set if Session Description Protocol Decurity"
          " Description (SDES) is used",
          DEFAULT_USE_SDES, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_MASTER_KEY,
      g_param_spec_string ("master-key", "Master key",
          "Master key (either 30 or 46 bytes, depending on the"
          " crypto-suite used)",
          DEFAULT_MASTER_KEY,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_CRYPTO_SUITE,
      g_param_spec_enum ("crypto-suite", "Crypto suite",
          "Describes the encryption and authentication algorithms",
          KMS_TYPE_RTP_SDES_CRYPTO_SUITE, DEFAULT_CRYPTO_SUITE,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

  obj_signals[SIGNAL_KEY_SOFT_LIMIT] = g_signal_new ("key-soft-limit",
...
kms_rtp_endpoint_init (KmsRtpEndpoint *self)
{
  ...
  GST_WARNING ("=============kms_rtp_endpoint_init===============");
  self->priv = KMS_RTP_ENDPOINT_GET_PRIVATE (self);

  self->priv->sdes_keys = g_hash_table_new_full (
      g_str_hash, g_str_equal, g_free, (GDestroyNotify) kms_ref_struct_unref);

  self->priv->comedia.rtp_conns =
      g_hash_table_new_full (NULL, NULL, g_object_unref, NULL);
  self->priv->comedia.signal_ids =
      g_hash_table_new_full (NULL, NULL, g_object_unref, NULL);

  g_object_set (G_OBJECT (self), "bundle", FALSE, "rtcp-mux", FALSE,
      "rtcp-nack", TRUE, "rtcp-remb", TRUE, "max-video-recv-bandwidth", 0,
      NULL);
...
//linux is not use this 
gboolean
kms_rtp_endpoint_plugin_init (GstPlugin *plugin)
{
  GST_WARNING ("=============kms_rtp_endpoint_plugin_init %s===============",
      PLUGIN_NAME);
  return gst_element_register (
      plugin, PLUGIN_NAME, GST_RANK_NONE, KMS_TYPE_RTP_ENDPOINT);
}

GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, kmsrtpendpoint,
    "Kurento rtp endpoint", kms_rtp_endpoint_plugin_init, VERSION,
    GST_LICENSE_UNKNOWN, "Kurento Elements", "http://kurento.com/")
